...
@pytest.mark.asyncio
async def test_valid_session_proceeds(client: AsyncClient):
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (session_id, now, now + timedelta(days=20)),
        )
...
@pytest.mark.asyncio
async def test_near_expiry_session_extended(client: AsyncClient):
    ...
    old_expires = now + timedelta(days=3)  # Within 7-day threshold
...
@pytest.mark.asyncio
async def test_far_from_expiry_not_updated(client: AsyncClient):
    ...
    original_expires = now + timedelta(days=20)  # Outside threshold
...
async def test_expired_session_gets_new_cookie(client: AsyncClient):
    ...
    with get_db() as conn:
        conn.execute(
            "INSERT INTO sessions (id, created_at, expires_at) VALUES (%s, %s, %s)",
            (expired_id, now - timedelta(days=10), expired_at),
        )
...
@pytest.mark.asyncio
async def test_malformed_cookie_no_db_query(client: AsyncClient):
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT id FROM sessions WHERE id = %s", ("not-a-uuid-at-all",)
        ).fetchone()
    assert row is None
...
@pytest.mark.asyncio
async def test_absolute_lifetime_expired(client: AsyncClient):
...
def test_session_rotation_migrates_cart(app):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            "INSERT INTO sessions (id, created_at, expires_at) VALUES (%s, %s, %s)",
            (old_session, now, now + timedelta(days=30)),
        )
        ...
        row = conn.execute("SELECT user_id FROM sessions WHERE id = %s", (new_session,)).fetchone()
        assert row is not None
        assert row["user_id"] == user_id

        # Verify old session deleted
        old_row = conn.execute("SELECT id FROM sessions WHERE id = %s", (old_session,)).fetchone()
        assert old_row is None

        # Verify cart items migrated with correct quantities
        items = conn.execute(
            "SELECT product_id, quantity FROM cart_items WHERE session_id = %s ORDER BY product_id",
            (new_session,),
        ).fetchall()
        assert len(items) == 3
...
@pytest.mark.asyncio
async def test_logout_sets_single_rotated_session_cookie(client: AsyncClient):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            ...
            (old_session, user.id, now, now + timedelta(days=30)),
        )
...
async def test_db_failure_returns_500(client: AsyncClient):
    ...
    @contextmanager
    def broken_get_db():
        raise DatabaseError("DB is broken")
...
def test_session_rotation_rollback_on_failure(app):
    ...
    with get_db() as conn:
        ...
        items = conn.execute(
            "SELECT product_id, quantity FROM cart_items WHERE session_id = %s",
            (old_session,),
        ).fetchall()
        assert len(items) == 1
