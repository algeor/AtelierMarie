...
@pytest.fixture()
def cart_db(service_db: psycopg.Connection) -> psycopg.Connection:
    """Return a pooled connection with test products and a session set up."""
    conn = service_db

    # Session (FAKE_SESSION_ID == "test-session" is already re-inserted by
    # the autouse _clean_tables fixture, so tolerate the pre-existing row).
    conn.execute(
...
class TestGetCart:
    ...
    def test_empty_cart(self, cart_db: psycopg.Connection):
    ...
    def test_cart_with_items_totals(self, cart_db: psycopg.Connection):
    ...
    def test_cart_item_product_includes_taxonomy_metadata(self, cart_db: psycopg.Connection):
        """Cart product payloads expose managed taxonomy, including labels."""
        cart_db.execute(
            "UPDATE products SET product_type_slug = %s, category_slug = %s WHERE id = %s",
    ...
    def test_inactive_product_in_unavailable(self, cart_db: psycopg.Connection):
...
class TestAddItem:
    ...
    def test_add_new_item_created_true(self, cart_db: psycopg.Connection):
    ...
    def test_add_existing_item_created_false(self, cart_db: psycopg.Connection):
    ...
    def test_add_product_not_found(self, cart_db: psycopg.Connection):
        """Adding a non-existent product raises ProductNotFoundError."""
        with pytest.raises(ProductNotFoundError):
            add_item(cart_db, SESSION_ID, "nonexistent-product", 1)

    def test_add_inactive_product_not_found(self, cart_db: psycopg.Connection):
...
class TestAddItemLimits:
    ...
    def test_per_item_limit_exceeded(self, cart_db: psycopg.Connection):
    ...
    def test_cart_full(self, cart_db: psycopg.Connection):
    ...
    def test_existing_item_when_cart_full(self, cart_db: psycopg.Connection):
...
class TestUpdateQuantity:
    ...
    def test_valid_update(self, cart_db: psycopg.Connection):
        """Valid update changes quantity."""
        add_item(cart_db, SESSION_ID, "lavender-dream", 2)
        cart = update_quantity(cart_db, SESSION_ID, "lavender-dream", 5)
        assert cart.items[0].quantity == 5

    def test_update_to_zero_removes(self, cart_db: psycopg.Connection):
    ...
    def test_update_item_not_in_cart(self, cart_db: psycopg.Connection):
...
class TestRemoveItem:
    ...
    def test_remove_existing(self, cart_db: psycopg.Connection):
        """Removing existing item succeeds."""
        add_item(cart_db, SESSION_ID, "lavender-dream", 2)
        cart = remove_item(cart_db, SESSION_ID, "lavender-dream")
        assert len(cart.items) == 0

    def test_remove_nonexistent_raises(self, cart_db: psycopg.Connection):
...
def test_total_cents_excludes_unavailable_items(cart_db: psycopg.Connection):
...
def test_concurrent_stock_depletion(app):
    """Two threads competing for stock=1 — each on its own connection.
    ...
    With stock=1 and two sessions each requesting qty=1 both may succeed (stock is
    never decremented by add_item). The invariant is per-session consistency: each
    session's cart_items quantity is stored without corruption.
    """
    ...
    with get_db() as conn:
        ...
        conn.execute(
            ...
            "VALUES ('race-product', 'Race Product', 1000, 1, 1, "
    ...
    # Stock is advisory: both sessions may get the item (stock never decremented).
...
def test_product_deactivated_after_add(cart_db: psycopg.Connection):
...
class TestBoundaryValues:
    ...
    def test_add_at_limit_succeeds(self, cart_db: psycopg.Connection):
    ...
    def test_add_over_limit_fails(self, cart_db: psycopg.Connection):
        """Existing qty 10 + add 1 = 11 fails."""
        add_item(cart_db, SESSION_ID, "ocean-breeze", 10)
        with pytest.raises(QuantityLimitError):
            add_item(cart_db, SESSION_ID, "ocean-breeze", 1)

    def test_update_to_exactly_limit(self, cart_db: psycopg.Connection):
    ...
    def test_update_over_limit_fails(self, cart_db: psycopg.Connection):
        """update_quantity to 11 fails."""
        add_item(cart_db, SESSION_ID, "ocean-breeze", 5)
        with pytest.raises(QuantityLimitError):
            update_quantity(cart_db, SESSION_ID, "ocean-breeze", 11)

    def test_cart_19_items_add_20th_succeeds(self, cart_db: psycopg.Connection):
    ...
    def test_cart_20_items_add_21st_fails(self, cart_db: psycopg.Connection):
...
def test_add_item_quantity_zero_raises(cart_db: psycopg.Connection):
...
def test_concurrent_per_item_limit(app):
    ...
    with get_db() as conn:
        conn.execute(
            "INSERT INTO sessions (id, created_at, expires_at) "
            "VALUES ('shared-session', CURRENT_TIMESTAMP, "
            "CURRENT_TIMESTAMP + INTERVAL '30 days')"
        )
        conn.execute(
            "INSERT INTO products (id, name_en, price_cents, stock, is_active, "
            "created_at, updated_at) "
            "VALUES ('limit-race', 'Limit Race', 1000, 20, 1, "
            "CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
        )
    ...
    assert row["quantity"] <= 10, f"Final quantity {row['quantity']} exceeds per-item limit of 10"
