...
describe("Econt fake-client flow", () => {
  it("runs checkout -> admin label -> customer tracking with mock Econt", async () => {
    ...
    expect(label.shipment_number).toBe("1234567890");
    expect(label.label_url).toBe("https://label.test/1234567890.pdf");

    const adminOrder = await getAdminOrder(order.id);
    expect(adminOrder.courier_provider).toBe("econt");
    expect(adminOrder.courier_shipment_number).toBe("1234567890");
    expect(adminOrder.tracking_url).toBe("https://www.econt.com/services/track-shipment/1234567890");
    ...
    expect(customerOrder.courier_shipment_number).toBe("1234567890");
    expect(customerOrder.tracking_carrier).toBe("econt");
    expect(customerOrder.tracking_url).toBe("https://www.econt.com/services/track-shipment/1234567890");
  });
});
