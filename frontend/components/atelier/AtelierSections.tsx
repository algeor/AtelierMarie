...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return <div className="mb-4 h-0.5 w-14 bg-muted-gold" aria-hidden="true" />;
}
...
export function Hero({ section }: { section: AboutSection }) {
  return (
    <SectionShell section={section} className="relative overflow-hidden bg-charcoal py-0 text-warm-ivory">
        ...
        <div className="relative z-10 flex min-h-[82vh] items-end px-4 pb-16 sm:px-6 lg:px-8">
          <div className="mx-auto w-full max-w-7xl">
            <div className="max-w-3xl">
              <h1 className="font-heading text-5xl text-warm-ivory sm:text-6xl lg:text-7xl">
                {section.heading}
              </h1>
              {section.subheading && (
      ...
      <div className="bg-warm-ivory px-4 py-14 text-charcoal sm:px-6 lg:px-8">
...
export function TextImage({ section }: { section: AboutSection }) {
  return (
    <SectionShell section={section} className="bg-warm-ivory">
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16 lg:px-8">
        <div className="overflow-hidden rounded-2xl bg-cream shadow-sm ring-1 ring-champagne-beige">
          <img src={imageFor(section)} alt="" className="aspect-[4/5] w-full object-cover" />
        </div>
        <div>
          <Eyebrow>{null}</Eyebrow>
          <h2 className="font-heading text-4xl text-charcoal sm:text-5xl">{section.heading}</h2>
          {section.subheading && <p className="mt-4 text-lg text-muted-gold">{section.subheading}</p>}
...
export function TextBand({ section }: { section: AboutSection }) {
  return (
    <SectionShell section={section} className="bg-cream">
      <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        <div className="mx-auto mb-5 h-0.5 w-14 bg-muted-gold" aria-hidden="true" />
        <h2 className="font-heading text-4xl text-charcoal sm:text-5xl">{section.heading}</h2>
        {section.subheading && <p className="mt-4 text-lg text-muted-gold">{section.subheading}</p>}
        <BodyRenderer body={section.body} className="mt-8" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {section.items.map((item) => (
            <article key={item.id} className="rounded-2xl bg-cream p-6 shadow-sm ring-1 ring-champagne-beige">
              <h3 className="font-heading text-2xl text-charcoal">{item.title}</h3>
              {item.text && <p className="mt-4 text-sm leading-7 text-soft-brown">{item.text}</p>}
            </article>
          ))}
        </div>
      </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
          ...
          <div className="space-y-4">
            {section.items.map((item, index) => (
              <article key={item.id} className="grid grid-cols-[3rem_1fr] gap-4 rounded-2xl bg-warm-ivory p-5 shadow-sm ring-1 ring-champagne-beige">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted-gold text-sm font-semibold text-warm-ivory">
                  {String(index + 1).padStart(2, "0")}
                </div>
                <div>
                  <h3 className="font-heading text-2xl text-charcoal">{item.title}</h3>
                  {item.text && <p className="mt-2 text-sm leading-7 text-soft-brown">{item.text}</p>}
                </div>
              </article>
            ))}
          </div>
    ...
    <SectionShell section={section} className="bg-warm-ivory">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {section.items.map((item) => {
            const content = (
              <article className="group overflow-hidden rounded-2xl bg-cream shadow-sm ring-1 ring-champagne-beige">
                <img src={imageFor(section, item.image)} alt="" className="aspect-[4/3] w-full object-cover transition-transform duration-300 group-hover:scale-105" />
                <div className="p-6">
                  <h3 className="font-heading text-2xl text-charcoal">{item.title}</h3>
                  {item.text && <p className="mt-3 text-sm leading-7 text-soft-brown">{item.text}</p>}
                </div>
              </article>
            );
...
export function CtaBand({ section }: { section: AboutSection }) {
  return (
    <SectionShell section={section} className="bg-charcoal text-warm-ivory">
      <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        <h2 className="font-heading text-4xl text-warm-ivory sm:text-5xl">{section.heading}</h2>
        <BodyRenderer body={section.body} className="mt-6 text-cream" />
        <div className="mt-9"><Cta section={section} /></div>
      </div>
    </SectionShell>
  );
