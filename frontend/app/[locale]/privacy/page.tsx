  ...
  return (
    <main className="overflow-x-hidden bg-page text-text">
      <section id="privacy-top" className="mx-auto max-w-5xl scroll-mt-28 px-4 pb-8 pt-12 sm:px-6 sm:pt-16 lg:px-8">
        <p className="text-sm font-medium uppercase tracking-[0.08em] text-accent">{privacy.eyebrow}</p>
        <div className="mt-3 max-w-3xl">
          <h1 className="break-words font-heading text-3xl text-text sm:text-5xl">{privacy.title}</h1>
          <p className="mt-5 max-w-full break-words text-base leading-8 text-muted sm:text-lg">{privacy.subtitle}</p>
      ...
      <div className="mx-auto grid max-w-5xl min-w-0 gap-10 px-4 pb-16 sm:px-6 lg:grid-cols-[220px_minmax(0,760px)] lg:px-8">
        ...
        <article className="min-w-0 space-y-10 sm:space-y-12">
          <section className="min-w-0 rounded-brand border border-border/60 bg-surface-elevated/75 p-5 shadow-sm shadow-border/10">
            <h2 className="font-heading text-2xl text-text">{privacy.controllerTitle}</h2>
            ...
            <p className="mt-4 text-sm leading-6 text-muted/80">{legal.ownerReviewNotice}</p>
          </section>
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            <Link href={policyPath("cookies")} className="rounded-pill bg-surface px-4 py-2 text-muted underline-offset-4 hover:text-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{legal.cookiePolicy}</Link>
            <Link href={policyPath("contact")} className="rounded-pill bg-surface px-4 py-2 text-muted underline-offset-4 hover:text-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{legal.contactPage}</Link>
          </div>
        </article>
      </div>
    </main>
