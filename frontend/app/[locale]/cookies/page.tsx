  ...
  return (
    <main className="overflow-x-hidden bg-page text-text">
      <section id="cookies-top" className="mx-auto max-w-5xl scroll-mt-28 px-4 pb-8 pt-12 sm:px-6 sm:pt-16 lg:px-8">
        <p className="text-sm font-medium uppercase tracking-[0.08em] text-accent">{cookies.eyebrow}</p>
        <div className="mt-3 max-w-3xl">
          <h1 className="break-words font-heading text-3xl text-text sm:text-5xl">{cookies.title}</h1>
          <p className="mt-5 max-w-full break-words text-base leading-8 text-muted sm:text-lg">{cookies.subtitle}</p>
      ...
      <article className="mx-auto max-w-5xl px-4 pb-16 sm:px-6 lg:px-8">
        <section className="min-w-0 rounded-brand border border-border/60 bg-surface-elevated/75 p-5 shadow-sm shadow-border/10">
          <h2 className="font-heading text-2xl text-text">{cookies.inventoryTitle}</h2>
          <div className="mt-5 overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
              <thead className="text-text">
                <tr>
                  <th className="border-b border-border/60 px-3 py-2 font-medium">{cookies.headers.name}</th>
                  <th className="border-b border-border/60 px-3 py-2 font-medium">{cookies.headers.purpose}</th>
                  <th className="border-b border-border/60 px-3 py-2 font-medium">{cookies.headers.type}</th>
                  <th className="border-b border-border/60 px-3 py-2 font-medium">{cookies.headers.duration}</th>
                </tr>
              </thead>
              <tbody className="text-muted">
                {cookies.cookies.map((item) => (
                  <tr key={item.name}>
                    <td className="border-b border-border/60 px-3 py-3 font-medium text-text">{item.name}</td>
                    <td className="border-b border-border/60 px-3 py-3">{item.purpose}</td>
                    <td className="border-b border-border/60 px-3 py-3">{item.type}</td>
                    <td className="border-b border-border/60 px-3 py-3">{item.duration}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="mt-12 space-y-10 sm:space-y-12">
          {cookies.sections.map((section) => (
            <section key={section.id} id={section.id} className="min-w-0 scroll-mt-28 border-b border-border/60 pb-10 last:border-b-0">
              <h2 className="break-words font-heading text-2xl text-text sm:text-3xl">{section.title}</h2>
        ...
        <div className="mt-8 flex flex-wrap gap-3 text-sm">
          <Link href={policyPath("privacy")} className="rounded-pill bg-surface px-4 py-2 text-muted underline-offset-4 hover:text-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{legal.privacyPolicy}</Link>
          <Link href={policyPath("terms")} className="rounded-pill bg-surface px-4 py-2 text-muted underline-offset-4 hover:text-text hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{legal.termsConditions}</Link>
        </div>
      </article>
    </main>
  );
