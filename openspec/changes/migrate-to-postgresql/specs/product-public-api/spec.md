...
#### Scenario: Search uses locale-appropriate tsvector index
- **WHEN** `GET /v1/products?q=лавандула&locale=bg` is called
- **THEN** the system executes `WHERE search_bg @@ plainto_tsquery('simple', $1)` against the Bulgarian `tsvector` column and returns matching products with BG content

#### Scenario: Search in English (default)
- **WHEN** `GET /v1/products?q=lavender` is called without locale parameter
- **THEN** the system executes `WHERE search_en @@ plainto_tsquery('english', $1)` and returns matching products ordered by `ts_rank_cd DESC` with EN content

#### Scenario: Search input containing operator characters is safe
- **WHEN** `GET /v1/products?q=lavender%20%26%20poison` (i.e. `q=lavender & poison`) is called
- **THEN** `plainto_tsquery` interprets the entire input as literal words and no SQL injection or unintended operator parsing occurs

### Requirement: Get product detail endpoint
The system SHALL expose `GET /v1/products/{product_id}` returning a single active product. The endpoint SHALL accept an optional `locale` query parameter (one of: `en`, `bg`, default `en`). The response SHALL return product name and description in the requested locale with fallback to the other language. The endpoint SHALL return 404 if the product does not exist or is inactive. Product IDs remain text (SKU/slug); no ID format changes as part of the Postgres migration.
