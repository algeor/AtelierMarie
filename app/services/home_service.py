"""Service layer for editable homepage content."""
...
class HomeSectionNotFoundError(Exception):
    """Raised when a homepage section slug does not exist."""


class HomeItemNotFoundError(Exception):
    """Raised when a homepage item id does not exist under the requested section."""


class HomeReorderError(Exception):
    """Raised when a reorder request does not match the current row set."""


class HomeValidationError(Exception):
    """Raised for invalid editable homepage content."""
...
def _section_owner_slug(slug: str) -> str:
    return f"home-{slug.replace('_', '-')}"


def _item_owner_slug(item_id: int) -> str:
    return f"home-item-{item_id}"
...
def _sanitize_required(value: str) -> str:
    ...
    if sanitized is None:
        raise HomeValidationError("Required text cannot be blank")
    return sanitized
...
def _clean_url(value: str | None) -> str | None:
    ...
    if not is_safe_http_or_relative_url(stripped):
        raise HomeValidationError("URL must be http(s) or a safe relative URL")
    return stripped
...
def get_public_home(locale: str = "en") -> dict:
    """Return published localized homepage sections and published items."""
    localized = locale == "bg"
    section_select = (
...
def _ensure_section_exists(conn: DbConnection, slug: str) -> None:
    row = conn.execute("SELECT 1 FROM home_sections WHERE slug = %s", (slug,)).fetchone()
    if row is None:
        raise HomeSectionNotFoundError(f"Homepage section not found: {slug}")


def _ensure_item_exists(conn: DbConnection, section: str, item_id: int) -> None:
    row = conn.execute(
        "SELECT 1 FROM home_items WHERE section = %s AND id = %s", (section, item_id)
    ).fetchone()
    if row is None:
        raise HomeItemNotFoundError(f"Homepage item not found: {item_id}")
...
def update_section_text(slug: str, updates: dict) -> dict:
    ...
    for key, value in updates.items():
        if key in _SECTION_TEXT_FIELDS:
            ...
            raise HomeValidationError(f"Field cannot be edited: {key}")
    ...
    with get_db() as conn:
        ...
        if fields:
            ...
            conn.execute(f"UPDATE home_sections SET {set_clause} WHERE slug = %s", [*values, slug])
    return next(s for s in list_admin_home()["sections"] if s["slug"] == slug)


def create_item(section: str, payload: dict) -> dict:
    """Create a homepage item with the next sort order under a section."""
    fields = {
    ...
    with get_db() as conn:
        ...
        row = conn.execute(
            "SELECT COALESCE(MAX(sort_order), -1) AS max_order FROM home_items WHERE section = %s",
            (section,),
        ).fetchone()
        ...
        if item_id is None:
            raise RuntimeError("Homepage item insert did not return an id")
...
def get_admin_item(section: str, item_id: int) -> dict:
    with get_db() as conn:
        row = conn.execute(
            "SELECT * FROM home_items WHERE section = %s AND id = %s", (section, item_id)
        ).fetchone()
        if row is None:
            raise HomeItemNotFoundError(f"Homepage item not found: {item_id}")
...
def update_item(section: str, item_id: int, updates: dict) -> dict:
    """Patch editable fields on a homepage item."""
    fields: dict[str, object] = {}
    for key, value in updates.items():
    ...
    with get_db() as conn:
        ...
        if fields:
            ...
            conn.execute(
                f"UPDATE home_items SET {set_clause}, updated_at = CURRENT_TIMESTAMP "
...
def delete_item(section: str, item_id: int) -> None:
    with get_db() as conn:
        _ensure_item_exists(conn, section, item_id)
        conn.execute("DELETE FROM home_items WHERE section = %s AND id = %s", (section, item_id))


def reorder_sections(slugs: list[str]) -> list[dict]:
    """Replace section sort order; submitted set must match current sections."""
    with get_db() as conn:
        rows = conn.execute("SELECT slug FROM home_sections ORDER BY sort_order, slug").fetchall()
        current = [row["slug"] for row in rows]
        if set(current) != set(slugs) or len(current) != len(slugs):
            raise HomeReorderError("slugs must match all homepage sections")
        for sort_order, slug in enumerate(slugs):
            conn.execute(
                "UPDATE home_sections SET sort_order = %s, updated_at = CURRENT_TIMESTAMP "
                "WHERE slug = %s",
                (sort_order, slug),
            )
    return list_admin_home()["sections"]
...
def reorder_items(section: str, ids: list[int]) -> list[dict]:
    ...
    with get_db() as conn:
        _ensure_section_exists(conn, section)
        rows = conn.execute(
            "SELECT id FROM home_items WHERE section = %s ORDER BY sort_order, id", (section,)
        ).fetchall()
        current = [int(row["id"]) for row in rows]
        if set(current) != set(ids) or len(current) != len(ids):
            raise HomeReorderError("ids must match all items for the section")
        for sort_order, item_id in enumerate(ids):
            conn.execute(
                "UPDATE home_items SET sort_order = %s, updated_at = CURRENT_TIMESTAMP "
                "WHERE section = %s AND id = %s",
...
def set_section_published(slug: str, is_published: bool) -> dict:
    with get_db() as conn:
        _ensure_section_exists(conn, slug)
        conn.execute(
            "UPDATE home_sections SET is_published = %s, updated_at = CURRENT_TIMESTAMP "
            "WHERE slug = %s",
...
def set_item_published(section: str, item_id: int, is_published: bool) -> dict:
    with get_db() as conn:
        _ensure_item_exists(conn, section, item_id)
        conn.execute(
            "UPDATE home_items SET is_published = %s, updated_at = CURRENT_TIMESTAMP "
            "WHERE section = %s AND id = %s",
...
def set_item_image(section: str, item_id: int, file_bytes: bytes) -> dict:
    ...
    with get_db() as conn:
        ...
        process_image(file_bytes, owner_slug, image_id=image_id)
        conn.execute(
            "UPDATE home_items SET image_id = %s, updated_at = CURRENT_TIMESTAMP "
            "WHERE section = %s AND id = %s",
...
def clear_item_image(section: str, item_id: int) -> dict:
    with get_db() as conn:
        _ensure_item_exists(conn, section, item_id)
        conn.execute(
            "UPDATE home_items SET image_id = NULL, updated_at = CURRENT_TIMESTAMP "
            "WHERE section = %s AND id = %s",
            (section, item_id),
        )
    return get_admin_item(section, item_id)
